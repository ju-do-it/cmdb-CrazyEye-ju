...
class AdminSite(object):
    def __init__(self, name='admin'):
        self.registered_sites = {}       # model_class class -> admin_class instance
        self.name = name
    ...
    def register(self,model_class,admin_class=None):
        ...
        if app_name  not in self.registered_sites:
            self.registered_sites[app_name] = {}  #enabled_admins['crm'] = {}

        if model_name in self.registered_sites[app_name]:
            raise AdminAlreadyRegistered("app [%s] model [%s] has already registered!" %(app_name,model_name))
