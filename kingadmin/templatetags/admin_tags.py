...
def load_search_element(table_obj):
    ...
    if table_obj.search_fields:
        ...
        placeholder = "搜索 关键字 %s" % ",".join(table_obj.search_fields)
        ele = '''
           <hr>
           <div class="row" >
             <form method='get' role="search">
                    ...
                    <div class="col-md-2" >
					   <button type="submit" style="margin-top:15px;" class="btn btn-success">搜索</button>
					</div>

			 </form>
           <div>

        ''' % (table_obj.request.GET.get('q') if table_obj.request.GET.get('q') else '', placeholder,already_exist_ars)
    ...
    #                         <button class="btn btn-success" value="搜索" type="button"> <i class="fa fa-search"></i></button>
    #                         <input type="submit"  style="margin-top:18px" class="btn btn-success" value="开始过滤">
    #                     </span>
    #                 </div>
    #            </form>
    #        </div>
...
def get_db_table_name(table_admin_class):
    ...
    # 这是注意的是
    print("======able_admin_class=====",table_admin_class)
    # ======able_admin_class===== <kingadmin.admin_base.BaseKingAdmin object at 0x0000023D28169A58>
    # ======able_admin_class===== <kingadmin.admin_base.BaseKingAdmin object at 0x0000023D28169A90>
    # ======able_admin_class===== <kingadmin.admin_base.BaseKingAdmin object at 0x0000023D28169AC8>

    print("======able_admin_class=====",table_admin_class.model._meta)
    #     ======able_admin_class===== repository.asset
    # ======able_admin_class===== repository.networkdevice
    # ======able_admin_class===== repository.server

    # verbose_name_plural 和 verbose_name 在models的是啥设置。
    return table_admin_class.model._meta.verbose_name_plural.capitalize()  # 在前端中返回了 verbose_name_plural 定义的中文名。
...
def load_admin_actions(table_obj):
    ...
    select_ele = "<select id='admin_action' name='admin_action' class='form-control' > Action:<option value=''>-------</option>"
...
def add_new_obj_btn(form_obj ,field):
    ...
    #print("add_new_obj_btn site enabled ",site.registered_sites)
    ...
    if field_type in ("ForeignKey", "ManyToManyField"):
        ...
        if app_label in site.registered_sites:
            if model_name in site.registered_sites.get(app_label): #make sure this class is registered
...
@register.simple_tag
def get_app_name(admin_obj):
    return admin_obj.model._meta.app_label


@register.simple_tag
def get_model_name(model_obj):
    return model_obj.model._meta.model_name
