...
def app_index(request):
    print('=========site.registered_sites=========', site.registered_sites)
    # =========site.registered_sites========= {'repository': {'userprofile': <class 'kingadmin.admin_base.BaseKingAdmin'>,
    #        'nic': <class 'kingadmin.admin_base.BaseKingAdmin'>, 'usergroup': <class 'kingadmin.admin_base.BaseKingAdmin'>,
    #        'disk': <class 'kingadmin.admin_base.BaseKingAdmin'>, 'memory': <class 'kingadmin.admin_base.BaseKingAdmin'>,
    #        'server': <class 'kingadmin.admin_base.BaseKingAdmin'>, 'articledetail': <class 'kingadmin.admin_base.BaseKingAdmin'>,}}
    for k,app_name_obj in site.registered_sites.items():
        # print(k, app_name_obj.model._meta.verbose_name.capitalize())
        print('======app_name_obj======', app_name_obj)
        for k1,kingadmin_table_class in app_name_obj.items():
           print (k1,kingadmin_table_class,'=====',kingadmin_table_class.model._meta.verbose_name )
           # print (kingadmin_table_class.model._meta.verbose_name.capitalize())

    return render(request,'kingadmin/app_index.html', {'registered_sites_dict': site.registered_sites})


# @login_required
def app_tables(request,app_name):

    return render(request, 'kingadmin/app_index.html', {'registered_sites_dict':site.registered_sites,'current_app':app_name})
...
def display_table_list(request,app_name,table_name):
    errors = []
    if app_name in site.registered_sites:
        ##print(registered_sites[url])
        if table_name in site.registered_sites[app_name]:
            admin_class = site.registered_sites[app_name][table_name]

            if request.method == "POST":  # action 来了
                print(request.POST)

                editable_data = request.POST.get("editable_data")
                print('====editable_data======',editable_data)
                ...
                if editable_data: # for list editable
                    ...
                    print("=== selected_ids.split(',') ====",selected_ids.split(','))
            ...
            return render(request,'kingadmin/table_data_list.html',
                                                    {'table_obj':table_obj,
                                                     'app_name':app_name,
...
def table_change(request,app_name,table_name,obj_id):
    ...
    if app_name in site.registered_sites:
        if table_name in site.registered_sites[app_name]:
            admin_class = site.registered_sites[app_name][table_name]
            ##print(registered_sites[table_name])
            obj = admin_class.model.objects.get(id=obj_id)
...
def personal_password_reset(request):
    ...
    if request.method == "GET":
        change_form = site.registered_sites[app_name][model_name].add_form(instance=request.user)
    else:
        change_form = site.registered_sites[app_name][model_name].add_form(request.POST, instance=request.user)
...
def password_reset_form(request,app_name,table_db_name,user_id):
    ...
    if can_change_user_password:
        if request.method == "GET":
            change_form = site.registered_sites[app_name][table_db_name].add_form(instance=user_obj)
        else:
            change_form = site.registered_sites[app_name][table_db_name].add_form(request.POST,instance=user_obj)
            if change_form.is_valid():
                change_form.save()
                url = "/%s/" %request.path.strip("/password/")
                return redirect(url)
