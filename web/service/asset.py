...
class Asset(BaseServiceList):
    def __init__(self):
        ...
        table_config = [
            ...
            },
            {
                ...
                'display': 1,
                'text': {
                    'content': "<a href='/asset-{device_type_id}-{nid}.html'>查看详细</a> | <a href='/edit-article-{device_type_id}-{nid}.html'>编辑</a>",
    ...
    def assets_detail(device_type_id, asset_id):
        ...
        try:
            ...
            detail_id = models.Asset.objects.get(id=asset_id).articledetail.nid  # 获取文章的 nid
            response.article_data = models.ArticleDetail.objects.get(nid=detail_id)   # 获取文章的信息。
            print('======response.article=======',response.article_data.content)
    ...
    @staticmethod
    def article_detail(device_type_id, asset_id):
        response = BaseResponse()
        try:
            if device_type_id == '1' or device_type_id == '2':
                response = models.Asset.objects.filter(id=asset_id).first()
            #     response.data = models.Server.objects.filter(asset_id=asset_id).select_related('asset').first()
            # else:
            #     response.data = models.NetworkDevice.objects.filter(asset_id=asset_id).select_related('asset').first()
