...
class Tag(BaseServiceList):
    def __init__(self):
        ...
        table_config = [
            ...
            },
            {
                ...
                'attr': {'edit-enable': 'true', 'edit-type': 'input','name': 'cabinet_num', 'origin': '@cabinet_num'}
            },
            # {
            #     'q': None,
            #     'title': "选项",
    ...
    def fetch_tags(self, request):
        ...
        try:
            ...
            asset_count = models.Tag.objects.filter(conditions).count()   # 根据搜索条件统计搜索总数量
            page_info = PageInfo(request.GET.get('pager', None), asset_count)   # 使用 PageInfo 构造 分页
            asset_list = models.Tag.objects.filter(conditions).extra(select=self.extra_select).values(*self.values_list)[page_info.start:page_info.end]
